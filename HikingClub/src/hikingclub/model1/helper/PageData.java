...
@ToString(includeFieldNames = true) // toString() 메소드 자동 생성
@Getter @Setter 					// getter, setter 자동 생성
// @Slf4j 								// 'log'라는 이름의 Log4j 객체 자동 생성
public class PageData {

	/** Log4j 객체 생성 */
	private static Logger log = LoggerFactory.getLogger("PageData");
	
	/***** GET 파라미터로 처리할 값 *****/
	private int nowPage = 1; 		// 현재 페이지 번호

	/***** DB에서 조회한 결과값 *****/
	private int totalCount = 0; 	// 전체 글의 개수 조회

	/***** 개발자가 정의해야 하는 값 *****/
	private int listCount = 10; 	// 한 페이지에 보여질 글의 목록 수
	private int groupCount = 5; 	// 한 그룹에 표시할 페이지 번호의 개수
	...
	private int offset = 0; 		// MySQL의 Limit의 시작 위치
